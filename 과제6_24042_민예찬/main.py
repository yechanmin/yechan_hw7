...
class ParkingApp:
    def __init__(self):
        ...
        pygame.display.set_caption("🚗 RL 주차 시뮬레이터")
    ...
    def reset_all(self):
        ...
        self.episode        = self.parked = self.crashed = 0
        self.collision      = self.timeouts = 0
        self.recent_results = []
        for _ in range(PRETRAIN_EPISODES):
            self._train_one()
        self.start_new_demo()
    ...
    def _demo_action(self, state):
        ...
        if all(v == 0 for v in q) or random.random() < 0.10:
    ...
    def _draw_q_overlay(self):
        state  = self.env.get_state()
        q      = self.agent.get_q_values(state)
        cx, cy = self.env.car.pixel_center()
        offsets = {0:(0,-22), 1:(0,22), 2:(-30,0), 3:(30,0), 4:(0,0)}
        max_q = max(q)
        for a, (ox, oy) in offsets.items():
            v     = q[a]
            color = (0,150,70) if v > 0 else (190,30,30) if v < 0 else (120,120,120)
            if abs(v - max_q) < 1e-9 and v != 0:
                pygame.draw.circle(self.screen, (0,200,100), (cx+ox, cy+oy), 11, 2)
            lbl = self.font_q.render(f"{v:.1f}", True, color)
            self.screen.blit(lbl, (cx+ox-10, cy+oy-7))
    ...
    def _draw_arrow(self):
        ...
        cx, cy = self.env.car.pixel_center()
        ex, ey = cx + dx*22, cy + dy*22
        if dx or dy:
            pygame.draw.line(self.screen, BLACK, (cx, cy), (ex, ey), 3)
            pygame.draw.circle(self.screen, BLACK, (ex, ey), 4)
    ...
    def draw_info_panel(self):
        ...
        total    = max(self.parked + self.crashed + self.collision + self.timeouts, 1)
        ...
        self._txt(panel, f"주차 성공: {self.parked:,}",      x0, 28+ROW,   self.font_s, (40,150,80))
        self._txt(panel, f"NPC 충돌: {self.collision:,}",    x0, 28+ROW*2, self.font_s, (210,60,60))
        self._txt(panel, f"벽 충돌:  {self.crashed:,}",      x0, 28+ROW*3, self.font_s, (190,100,40))
        self._txt(panel, f"시간초과: {self.timeouts:,}",     x0, 28+ROW*4, self.font_s, (160,120,40))

        self._txt(panel, f"전체 성공률 {park_r*100:.1f}%",  x0, 28+ROW*5+4, self.font_s)
        self._bar(panel, x0, 28+ROW*6+2, COL-PAD*2, 10, park_r,
                  (70,190,110) if park_r > 0.4 else (200,100,70))
        self._txt(panel, f"최근200판 {recent_r*100:.1f}%",  x0, 28+ROW*7+6, self.font_s)
        self._bar(panel, x0, 28+ROW*8+4, COL-PAD*2, 10, recent_r,
                  (70,190,110) if recent_r > 0.4 else (200,100,70))
        ...
        self._txt(panel, self.last_result, x1, 28, self.font_b, RESULT_COLOR.get(self.last_result, BLACK))
        ...
        state = self.env.get_state()
        dx_b, dy_b, dist_b, near_wall, near_npc = state
        self._txt(panel, f"목표 Δ({dx_b:+d},{dy_b:+d})  거리버킷={dist_b}", x1, 104, self.font_s, (80,80,80))

        wall_c = (210,60,60) if near_wall else (80,160,100)
        npc_c  = (210,60,60) if near_npc  else (80,160,100)
        self._txt(panel, f"벽 인접: {'⚠ 위험' if near_wall else '✓ 안전'}", x1, 120, self.font_s, wall_c)
        self._txt(panel, f"NPC 인접: {'⚠ 위험' if near_npc else '✓ 안전'}",  x1, 136, self.font_s, npc_c)

        tx, ty = self.env.target
        cx, cy = self.env.car.pos
        self._txt(panel, f"차량({cx},{cy}) → 목표({tx},{ty})", x1, 152, self.font_s, (80,80,80))
        ...
        self._txt(panel, f"Q-table: {self.agent.q_table_size:,} 상태", x2, 74, self.font_s)
        self._txt(panel, f"α={self.agent.alpha}  γ={self.agent.gamma}", x2, 90, self.font_s)
        self._txt(panel, f"NPC 차량: {NUM_NPC_CARS}대", x2, 106, self.font_s, (180,60,60))

        if self.training:
            pygame.draw.rect(panel, (255,240,200), (x2, 124, COL-PAD*2, 22), border_radius=4)
            self._txt(panel, "⚡ 학습 중...", x2+4, 128, self.font_s, (180,100,0))
        if self.auto_play:
            yy = 148 if self.training else 124
            pygame.draw.rect(panel, (220,240,255), (x2, yy, COL-PAD*2, 22), border_radius=4)
            self._txt(panel, "▶ 자동 플레이", x2+4, yy+4, self.font_s, (0,90,180))
        ...
        for i, (label, active) in enumerate(keys):
            ...
            pygame.draw.rect(panel, bg,  (bx, KEY_Y, btn_w-4, 26), border_radius=5)
            pygame.draw.rect(panel, (160,160,175), (bx, KEY_Y, btn_w-4, 26), 1, border_radius=5)
            ts = self.font_s.render(label, True, fc)
            tw, th = ts.get_size()
            panel.blit(ts, (bx + (btn_w-4-tw)//2, KEY_Y + (26-th)//2))

        self.screen.blit(panel, (0, PY))

    def draw(self):
        self.screen.fill((200,200,210))
        ...
        while True:
            ...
            if self.training:
                for _ in range(TRAIN_EPISODES_PER_FRAME):
                    self._train_one()
                if now - self.last_step_t > STEP_DELAY_MS:
